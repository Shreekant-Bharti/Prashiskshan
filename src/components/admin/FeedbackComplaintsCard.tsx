...
const FeedbackComplaintsCard: React.FC<FeedbackComplaintsCardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "all" | "feedback" | "complaint" | "suggestion"
  >("all");
      ...
      <div className="flex space-x-1 mb-4 bg-gray-100 rounded-lg p-1">
        {[
          { key: "all", label: "All", count: feedbacks.length },
          {
            key: "feedback",
            label: "Feedback",
            count: feedbacks.filter((f) => f.type === "feedback").length,
          },
          {
            key: "complaint",
            label: "Complaints",
            count: feedbacks.filter((f) => f.type === "complaint").length,
          },
          {
            key: "suggestion",
            label: "Suggestions",
            count: feedbacks.filter((f) => f.type === "suggestion").length,
          },
        ].map((tab) => (
          <button
            key={tab.key}
              ...
              <div className="flex justify-between items-start mb-3">
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    <span
                      className={`flex items-center px-2 py-1 rounded-full text-xs font-medium border ${getTypeColor(
                        feedback.type
                      )}`}
                    >
                  ...
                  <div className="flex items-center space-x-4 text-xs text-gray-500">
                    <span>
                      By: {feedback.submittedBy} ({feedback.userType})
                    </span>
                ...
                <div className="flex space-x-2">
                    ...
                    feedback.status !== "closed" && (
                      <>
                        <motion.button
                          ...
                          whileTap={{ scale: 0.95 }}
                          onClick={() =>
                            updateFeedbackStatus(feedback.id, "in-progress")
                          }
                  ...
                  {feedback.status === "resolved" && (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      onClick={() =>
                        updateFeedbackStatus(feedback.id, "closed")
                      }
        ...
        >
          ...
          >
            <h3 className="text-lg font-bold mb-4">
              Respond to {selectedFeedback.type}
            </h3>
            ...
            <div className="flex space-x-2 mt-4">
              <button
                onClick={() =>
                  respondToFeedback(selectedFeedback.id, responseText)
                }
